...
{
    private const string GeneratedNamespace = "Akbura.Generated";
    ...
    public static string GetHintName(IAkcssModuleSymbol symbol)
    {
        if (symbol == null)
        {
        ...
        var identity = GetModuleIdentity(symbol);
        return $"Akbura.Akcss.{SanitizeHintPart(identity)}.{GetStableHash(identity):x8}.g.cs";
    }
        ...
        AkcssGenerationSourceMap sourceMap)
    {
        ...
        var identity = GetModuleIdentity(symbol);
        var moduleTypeName = $"__AkburaAkcssModule_{GetStableHash(identity):x8}";
        ...
        source.AppendLine("{");
        source.Append("    internal static class ").Append(moduleTypeName).AppendLine();
        source.AppendLine("    {");
        source.Append("        internal const string MetadataName = ")
            .Append(ToStringLiteral(symbol.MetadataName))
            .AppendLine(";");
        source.Append("        internal const string SourcePath = ")
            .Append(ToStringLiteral(symbol.Path ?? string.Empty))
            .AppendLine(";");
        ...
        IAkcssModuleSymbol module)
    {
        ...
        source.AppendLine();
        source.Append("        internal static readonly global::System.Collections.Immutable.ImmutableArray<")
        ...
        AkcssGenerationSourceMap sourceMap)
    {
        ...
        if (hasLineDirective)
        {
            AppendIndentedLine(
                source,
                statementIndentation,
                $"#line {lineNumber} {ToStringLiteral(path)}");
        }

        AppendIndentedLine(source, statementIndentation, statement);
        if (hasLineDirective)
        {
    ...
    private static string? CreateDynamicResourceAssignment(
        ...
        DynamicResourceBinding dynamicResource)
    {
        ...
        var converter =
            ...
            $"? global::Avalonia.AvaloniaProperty.UnsetValue : (object?)({value})";
        return
            ...
            $"{dynamicResource.KeyExpression}, converter: {converter})));";
    }

    private static string? CreateClrPropertyAssignment(
        AkburaPropertySymbol property,
        string targetName,
        string value)
    {
        ...
        return $"(({GetTypeName(clrProperty.ContainingType)}){targetName}).{EscapeIdentifier(clrProperty.Name)} = {value};";
    }

    private static string? CreateAvaloniaPropertyAssignment(
        AkburaPropertySymbol property,
        string targetName,
        string value)
    {
        ...
        return propertyReference == null
            ? null
            : $"((global::Avalonia.AvaloniaObject){targetName}).SetValue({propertyReference}, {value});";
    }

    private static string? CreateAttachedPropertyAssignment(
        AkburaPropertySymbol property,
        string targetName,
        string value)
    {
        ...
        return $"{GetMethodReference(setter)}(({GetTypeName(setter.Parameters[0].Type)}){targetName}, {value});";
    }
    ...
    private static string GetModuleIdentity(IAkcssModuleSymbol symbol)
    {
