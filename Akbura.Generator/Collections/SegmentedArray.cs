...
public static class SegmentedArray
{
    /// <seealso cref="Array.Clear(Array)"/>
    public static void Clear<T>(SegmentedArray<T> array)
        => Clear(array, 0, array.Length);

    /// <seealso cref="Array.Clear(Array, int, int)"/>
    public static void Clear<T>(SegmentedArray<T> array, int index, int length)
    {
    ...
    public static void Copy<T>(SegmentedArray<T> sourceArray, SegmentedArray<T> destinationArray, int length)
    {
        if (length == 0)
        {
            return;
        ...
        {
            ThrowHelper.LongerThanSrcArray(nameof(sourceArray));
        }

        if (length > destinationArray.Length)
        {
            ThrowHelper.LongerThanDestArray(nameof(destinationArray));
        }
    ...
    public static void Copy<T>(SegmentedArray<T> sourceArray, Array destinationArray, int length)
    {
        ...
        if (length > destinationArray.Length)
        {
            ThrowHelper.LongerThanSrcArray(nameof(destinationArray));
        }
    ...
    public static void Copy<T>(SegmentedArray<T> sourceArray, int sourceIndex, SegmentedArray<T> destinationArray, int destinationIndex, int length)
    {
        if (length < 0)
        {
            ThrowHelper.NeedNonNegNum(nameof(length));
        }

        if (sourceIndex < 0)
        {
            ThrowHelper.ArrayLB(nameof(sourceIndex));
        }
    ...
    {
        AkburaDebug.Assert(length > 0);
        AkburaDebug.Assert(sourceIndex >= 0);
        AkburaDebug.Assert(destinationIndex >= 0);
        AkburaDebug.Assert((uint)(sourceIndex + length) <= array.Length);
        AkburaDebug.Assert((uint)(destinationIndex + length) <= array.Length);
    ...
    {
        ThrowHelper.ThrowIfNull(destinationArray);

        if (typeof(T[]) != destinationArray.GetType() && destinationArray.Rank != 1)
        {
            ThrowHelper.RankMustMatch();
        }
        ...
        {
            ThrowHelper.ArrayLB(nameof(destinationIndex));
        }
    ...
    public static int LastIndexOf<T>(SegmentedArray<T> array, T value, int startIndex, int count, IEqualityComparer<T>? comparer)
    {
        if (array.Length == 0)
        {
            // Special case for empty array: -1 and 0 are valid startIndex values.
            if (startIndex is not (-1) and not 0)
            {
                ThrowHelper.IndexMustBeLess();
            }

            // If array is empty, count must be 0.
            if (count != 0)
            {
                ThrowHelper.CountOutOfRange();
            }
    ...
    private struct AlignedSegmentEnumerator<T>
    {
        ...
        public bool MoveNext()
        {
            ...
            if (_completed == 0)
            {
                ...
                AkburaDebug.Assert(offset == (_secondOffset & SegmentedArrayHelper.GetOffsetMask<T>()), "Aligned views must start at the same segment offset");
        ...
        public SegmentEnumerable(SegmentedArray<T> array, int offset, int length)
        {
            if (offset < 0 || length < 0 || (uint)(offset + length) > (uint)array.Length)
            {
                ThrowHelper.ThrowArgumentOutOfRangeException(
                    "Offset and length must be non-negative, and their sum must not exceed the array length.",
                    nameof(offset));
            }
    ...
    private struct SegmentEnumerator<T>
    {
        ...
        public bool MoveNext()
        {
            ...
            else
            {
                ...
                _current = segment.AsMemory()[..Math.Min(SegmentedArrayHelper.GetSegmentSize<T>(), _length - _completed)];
                _completed += _current.Length;
                return true;
            }
        }
