...
internal sealed partial class Lexer : IDisposable
{
	...
	private bool IsAkcssDirectiveStart()
	{
		if (TextWindow.PeekChar(1) != 'a' ||
			TextWindow.PeekChar(2) != 'k' ||
			TextWindow.PeekChar(3) != 'c' ||
			TextWindow.PeekChar(4) != 's' ||
			TextWindow.PeekChar(5) != 's')
		{
			return false;
		}
		...
		while (next != SlidingTextWindow.InvalidCharacter &&
			   SyntaxFacts.IsWhitespace(next))
		{
			offset++;
			next = TextWindow.PeekChar(offset);
		}

		return next == SlidingTextWindow.InvalidCharacter ||
			   next == '{' ||
			   next == '.' ||
			   next == '/';
	}
