...
internal struct SlidingTextWindow
{
    ...
    /// <summary>
    /// The optimal window size for Akbura is not yet well-established. 
    /// Unlike the Roslyn codebase, we currently lack large-scale empirical data 
    /// on average lexeme length or common tokenization patterns.
    ///
    /// In Roslyn the window is set to 4096 based on extensive statistics of real-world 
    /// source files (including many large embedded test strings). Akbura, however, is 
    /// designed around a more segmented and component-oriented style, where extremely 
    /// long contiguous lexemes are less common. Because of this, adopting Roslyn’s 
    /// 4096-byte window would likely be suboptimal for our typical usage patterns.
    ///
    /// For now, a conservative value of 1024 is used. This value is subject to change 
    ...
    public readonly ReadOnlySpan<char> CurrentWindowSpan
    {
        get
        {
            ...
            return start < 0 || start >= _characterWindow.Count ? default : (ReadOnlySpan<char>)_characterWindow.AsSpan(start);
        }
    }
    /// <summary>
