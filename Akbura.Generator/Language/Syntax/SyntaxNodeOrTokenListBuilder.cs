...
internal sealed class SyntaxNodeOrTokenListBuilder
{
    ...
    public SyntaxNodeOrToken this[int index]
    {
        get
        {
            ...
            Debug.Assert(innerNode is not null);
            if (innerNode!.IsToken == true)
            {
                // getting public token so we do not know the position
    ...
    {
        Debug.Assert(item.UnderlyingNode is not null);
    ...
    public SyntaxNodeOrTokenList ToList()
    {
        if (_count > 0)
        {
            switch (_count)
            {
                case 1:
                    if (_nodes[0]!.IsToken)
                    {
                        return new SyntaxNodeOrTokenList(
                            GreenSyntaxList.List([_nodes[0]!]).CreateRed(),
                            index: 0);
                    }
                    else
                    {
                ...
                case 2:
                    return new SyntaxNodeOrTokenList(
                        GreenSyntaxList.List(_nodes[0]!, _nodes[1]!).CreateRed(),
                        index: 0);
                case 3:
                    return new SyntaxNodeOrTokenList(
                        GreenSyntaxList.List(_nodes[0]!, _nodes[1]!, _nodes[2]!).CreateRed(),
                        index: 0);
                default:
                    ...
                    return new SyntaxNodeOrTokenList(GreenSyntaxList.List(tmp).CreateRed(), index: 0);
            }
        }
        else
        {
