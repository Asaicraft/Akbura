...
internal readonly partial struct SyntaxTriviaList : IEquatable<SyntaxTriviaList>, IReadOnlyList<SyntaxTrivia>
{
    ...
    private static GreenNode? CreateNodeFromSpan(ReadOnlySpan<SyntaxTrivia> trivias)
    {
        switch (trivias.Length)
        {
            ...
            case 2: return GreenSyntaxList.List(trivias[0].UnderlyingNode!, trivias[1].UnderlyingNode!);
            case 3: return GreenSyntaxList.List(trivias[0].UnderlyingNode!, trivias[1].UnderlyingNode!, trivias[2].UnderlyingNode!);
            default:
            {
    ...
    public TextSpan FullSpan
    {
        get
        {
            ...
            return new(Position, Node.FullWidth);
        }
    }
    ...
    public SyntaxTriviaList Insert(int index, SyntaxTrivia trivia)
    {
        ...
        return InsertRange(index, [trivia]);
    }

    private static readonly ObjectPool<SyntaxTriviaListBuilder> s_builderPool = new(() => SyntaxTriviaListBuilder.Create());
    ...
    public SyntaxTriviaList RemoveAt(int index)
    {
        ...
        return new(default, GreenNode.CreateList(list, static n => n.RequiredUnderlyingNode), 0, 0);
    }
    ...
    public SyntaxTriviaList Replace(SyntaxTrivia triviaInList, SyntaxTrivia newTrivia)
    {
        ...
        return ReplaceRange(triviaInList, [newTrivia]);
    }
    ...
    IEnumerator<SyntaxTrivia> IEnumerable<SyntaxTrivia>.GetEnumerator()
    {
        if (Node == null)
        {
            return EmptyEnumerator.For<SyntaxTrivia>();
        }
    ...
    {
        return HashCode.Combine(Token, Node, Index);
    }
