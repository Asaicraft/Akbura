...
internal sealed class StringTable
{
    ...
    {
        var pool = new ObjectPool<StringTable>(pool => new StringTable(pool), 8);
        return pool;
    }
    ...
    {
        var hashCode = HashCode.GetFNVHashCode(chars);
    ...
    {
        var hashCode = HashCode.GetFNVHashCode(bytes, out var isAscii);
    ...
    internal static bool TextEqualsASCII(string text, ReadOnlySpan<byte> ascii)
    {
        ...
        {
            AkburaDebug.Assert((ascii[i] & 0x80) == 0, $"The {nameof(ascii)} input to this method must be valid ASCII.");
        }
#endif
