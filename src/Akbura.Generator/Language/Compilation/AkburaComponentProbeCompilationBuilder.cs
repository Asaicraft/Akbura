...
internal static class AkburaComponentProbeCompilationBuilder
{
    ...
    {
        compilation = RemoveGeneratedComponentTrees(
            compilation,
            syntaxTrees,
            rootNamespace,
            projectDirectory);
    ...
    private static CSharpCompilation RemoveGeneratedComponentTrees(CSharpCompilation compilation, ImmutableArray<AkburaSyntaxTree> syntaxTrees, string rootNamespace, string projectDirectory)
    {
        using var metadataNames = ImmutableArrayBuilder<string>.Rent();
        using var generatedHintNames = ImmutableArrayBuilder<string>.Rent();
        foreach (var syntaxTree in syntaxTrees)
        {
            if (GlobalUsings.IsComponentFile(syntaxTree) ||
            ...
            generatedHintNames.Add(ComponentDocumentWriter.GetHintName(
                metadataName,
                syntaxTree.FilePath));
        }

        var currentMetadataNames = metadataNames.ToImmutable();
        var currentGeneratedHintNames = generatedHintNames.ToImmutable();
        if (currentMetadataNames.IsDefaultOrEmpty)
        {
            return compilation;
        }

        using var generatedTrees = ImmutableArrayBuilder<SyntaxTree>.Rent();
        var seenGeneratedTrees = new HashSet<SyntaxTree>();
        for (var index = 0; index < currentMetadataNames.Length; index++)
        {
            var componentType = compilation.GetTypeByMetadataName(currentMetadataNames[index]);
            if (componentType == null)
            {
                continue;
            }

            foreach (var syntaxReference in componentType.DeclaringSyntaxReferences)
            {
                var syntaxTree = syntaxReference.SyntaxTree;
                if (IsAkburaGeneratedComponentTree(
                        syntaxTree,
                        currentGeneratedHintNames[index]) &&
                    seenGeneratedTrees.Add(syntaxTree))
                {
                    generatedTrees.Add(syntaxTree);
                }
            }
        }

        var treesToRemove = generatedTrees.ToImmutable();
        return treesToRemove.IsDefaultOrEmpty
            ? compilation
            : compilation.RemoveSyntaxTrees(treesToRemove);
    }

    private static bool IsAkburaGeneratedComponentTree(SyntaxTree syntaxTree, string expectedHintName)
    {
        var fileName = Path.GetFileName(syntaxTree.FilePath);
        if (!fileName.StartsWith("Akbura.Component.", StringComparison.Ordinal) ||
        ...
        if (!string.Equals(fileName, expectedHintName, StringComparison.Ordinal) &&
        ...
        var leadingTrivia = syntaxTree
            .GetRoot()
            .GetFirstToken(includeZeroWidth: true)
            .LeadingTrivia;
        return leadingTrivia.Any(static trivia =>
            trivia.IsKind(CSharpSyntaxKind.SingleLineCommentTrivia) &&
