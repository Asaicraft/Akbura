...
{
    private readonly AkburaSyntax? _parent;
    private readonly int _index;

    public SyntaxTokenList(AkburaSyntax? parent, GreenNode? tokenOrList, int position, int index)
    {
    ...
    private static GreenNode? CreateNodeFromSpan(ReadOnlySpan<SyntaxToken> tokens)
    {
        switch (tokens.Length)
        {
            ...
            case 2: return GreenSyntaxList.List(tokens[0].Node!, tokens[1].Node!);
            case 3: return GreenSyntaxList.List(tokens[0].Node!, tokens[1].Node!, tokens[2].Node!);
            default:
            {
    ...
    {
        Debug.Assert(Count >= offset + count);
    ...
    IEnumerator<SyntaxToken> IEnumerable<SyntaxToken>.GetEnumerator()
    {
        if (Node == null)
        {
            return EmptyEnumerator.For<SyntaxToken>();
        }
    ...
    public override int GetHashCode()
    {
        ...
        return HashCode.Combine(Node?.GetHashCode() ?? 0, _index);
    }
