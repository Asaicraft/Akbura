...
internal static class ArrayBuilderExtensions
{
    ...
    public static ImmutableArray<TResult> SelectAsArray<TItem, TArg, TResult>(this ArrayBuilder<TItem> items, Func<TItem, TArg, TResult> map, TArg arg)
    {
        switch (items.Count)
        {
            ...
            case 1:
                return [map(items[0], arg)];

            case 2:
                return [map(items[0], arg), map(items[1], arg)];

            case 3:
                return [map(items[0], arg), map(items[1], arg), map(items[2], arg)];

            case 4:
                return [map(items[0], arg), map(items[1], arg), map(items[2], arg), map(items[3], arg)];
