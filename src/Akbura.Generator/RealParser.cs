...
internal static class RealParser
{
    ...
    private abstract class FloatingPointType
    {
            ...
            out ulong result)
        {
            ...
            var normalMantissaShift = this.NormalMantissaBits - (int)initialMantissaBits;
            ...
            var shiftedExponent = ((ulong)(exponent + this.ExponentBias)) << DenormalMantissaBits;
...
#pragma warning restore IDE0251 // Make member 'readonly'
    ...
    private static Status ConvertDecimalToFloatingPointBits(DecimalFloatingPointString data, FloatingPointType type, out ulong result)
    {
        ...
        var requiredBitsOfPrecision = (uint)type.NormalMantissaBits + 1;
        ...
        var fractionalMantissa = (ulong)bigFractionalMantissa;
        ...
        if (CountSignificantBits(fractionalMantissa) > requiredFractionalBitsOfPrecision)
        {
            var shift = (int)(CountSignificantBits(fractionalMantissa) - requiredFractionalBitsOfPrecision);
        ...
        var integerMantissa = (ulong)integerValue;

        var completeMantissa =
    ...
    private static Status ConvertBigIntegerToFloatingPointBits(byte[] integerValueAsBytes, uint integerBitsOfPrecision, bool hasNonzeroFractionalPart, FloatingPointType type, out ulong result)
    {
        ...
        mantissa = 0;
        for (var i = (int)topElementIndex; i >= bottomElementIndex; i--)
        {
            mantissa <<= 8;
