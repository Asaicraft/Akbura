...
`Numbers` may resolve either to an `IList<int>` implementation or to a
non-generic enumerable such as `ArrayList`. Akbura validates every item and
copies the references into the component-owned typed backing collection:
...
events from the resolved collection continue to update the owned list. Use
`DynamicResource` when replacing the resource entry itself must reconnect the
parameter.

A plain `ArrayList` has no change notifications, so it is snapshotted once. If
application code mutates that same source instance, refresh it explicitly:

```csharp
ItemsPanel.DataProperty.RefreshSource(itemsPanel);
```

The public `Data` property remains `IList<int>`; assigning an untyped source
from C# uses the descriptor API instead of weakening that property:

```csharp
ItemsPanel.DataProperty.SetSource(itemsPanel, legacyArrayList);
```

## DataContext binding

Here `Items` may be an `IList<int>`, `System.Collections.IList`, or `ArrayList`
property on the inherited DataContext:
