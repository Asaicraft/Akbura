...
public sealed class WorkspaceCompletionTests
{
    ...
    private static void WithWorkspace(string source, string? stylesSource, Action<AkburaWorkspace, AkburaDocumentContext, AkburaSyntacticDocument> assertion, string? globalUsingsSource = null, MetadataReference? additionalReference = null, string? additionalCSharpSource = null, string? additionalCSharpPath = null)
    {
        var directory = Path.Combine(
            Path.GetTempPath(),
            nameof(WorkspaceCompletionTests),
        ...
        try
        {
            ...
            if (additionalCSharpSource != null)
            {
                compilation = compilation.AddSyntaxTrees(
                    CSharpSyntaxTree.ParseText(
                        additionalCSharpSource,
                        path: additionalCSharpPath ?? string.Empty));
            }
    ...
    [Fact]
    public void ProjectedCSharpService_PostGeneratedCommandUsesCurrentSourceContract()
    {
        const string sourceWithCaret =
            "namespace Gallery;\r\n" +
            "using Avalonia.Controls;\r\n\r\n" +
        ...
        const string generatedSource =
            ...
            {
                public global::Akbura.IAkburaCommand<global::Avalonia.Controls.Control, object> NavigateTo { get; set; } = null!;
            }
            """;
        var position = sourceWithCaret.IndexOf('|');
        var source = sourceWithCaret.Remove(position, 1);
        ...
        WithWorkspace(
            ...
            (workspace, semanticContext, syntacticDocument) =>
            {
                ...
                Assert.DoesNotContain(
                    diagnostics,
                    static diagnostic =>
                        diagnostic.Severity == Language.Syntax.AkburaDiagnosticSeverity.Error);
                ...
                var commandPosition = source.IndexOf(
                    "NavigateTo.Execute",
                    StringComparison.Ordinal);
                ...
                Assert.Equal(
                    Path.GetFullPath(semanticContext.Document.FilePath),
                    definition!.TargetFilePath);
                var declarationPosition = source.IndexOf(
                    "NavigateTo(Control",
                    StringComparison.Ordinal);
                Assert.Equal(
                    semanticContext.Document.Text.Lines.GetLinePositionSpan(
                        new TextSpan(declarationPosition, "NavigateTo".Length)),
                    definition.TargetLineSpan);
            },
            additionalCSharpSource: generatedSource,
            additionalCSharpPath:
                "Akbura.BlackSilence/Akbura.BlackSilence.AkburaBlackSilenceGenerator/" +
                "Akbura.Component.App.akbura.00000000.g.cs");
    }
